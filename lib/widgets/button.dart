  ...
  Widget build(BuildContext context) {
    final double resolvedBorderRadius =
        getPlatform() == PlatformOS.isIOS && widget.borderRadius == null
            ? 18
            : widget.borderRadius ?? 22;
    final bool useDefaultGradient =
        widget.color == null && widget.disabled == false;
    final Color backgroundBase = widget.disabled
        ? appStateSettings["materialYou"]
            ? Colors.grey
            : getColor(context, "lightDarkAccentHeavy")
        : widget.color ?? getCupertinoSurfaceColor(context, level: 1);
    final Color contentColor = widget.textColor ??
        (useDefaultGradient
            ? Colors.white
            : getCupertinoPrimaryTextColor(context));
          ...
          child: Padding(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: useDefaultGradient ? null : backgroundBase,
                ...
                border: Border.all(
                  color: useDefaultGradient
                      ? Theme.of(context).colorScheme.primary.withOpacity(0.16)
                      : getCupertinoBorderColor(context, opacity: 0.9),
                  width: 0.8,
                ),
                boxShadow: widget.disabled
                    ...
                    : getCupertinoShadow(
                        context,
                        elevation: useDefaultGradient ? 1.4 : 0.8,
                      ),
              ),
              child: Padding(
...
class ButtonIcon extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color resolvedColor = color ?? getCupertinoSurfaceColor(context);
    return Tappable(
      child: Container(
        height: size,
        width: size,
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: getCupertinoBorderColor(context),
            width: 0.8,
          ),
          boxShadow: getCupertinoShadow(context, elevation: 0.8),
        ),
        child: Icon(
          icon,
          color: iconColor ?? getCupertinoPrimaryTextColor(context),
...
class SelectedIconForIconButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: isSelected
            ? getCupertinoTintedSurfaceColor(
                context,
                Theme.of(context).colorScheme.tertiary,
                amount: 0.18,
              )
            : getCupertinoSurfaceColor(context, level: 0.4),
        borderRadius: BorderRadiusDirectional.circular(16),
        border: Border.all(
          color: isSelected
              ? Theme.of(context).colorScheme.tertiary.withOpacity(0.22)
              : getCupertinoBorderColor(context, opacity: 0.7),
          width: 0.8,
        ),
      ),
